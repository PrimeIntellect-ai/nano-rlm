"""Native ``bash`` and ``edit`` builtin tools (RLM_BUILTIN_TOOLS opt-in).

Replicates the verifiers bash harness interface inside rlm: ``bash`` runs one shell
command per call in a fresh subshell; ``edit`` does safe single-occurrence string
replacement. Enabled via ``RLM_BUILTIN_TOOLS`` (e.g. ``bash,edit`` or ``bash,ipython``).
"""
...
class EditTool:
    ...
    def execute(self, args: dict[str, Any], context: ToolContext) -> ToolOutcome:
        ...
        if count > 1:
            return ToolOutcome(content=f"Error: old_str occurs {count} times in {path}; must be unique")
