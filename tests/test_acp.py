...
async def test_tree_total_budget_stops_before_another_model_call(session):
    client = DummyClient(
        [
            DummyMessage(tool_calls=[DummyToolCall("add", {"a": 1, "b": 2})]),
            DummyMessage(content="should not run"),
        ]
    )
    config = make_runtime_config(
        policy=ExecutionPolicy(max_depth=0, max_total_tokens=2)
    )
    engine = RLMEngine(
        client=client,
        session=session,
    ...
    try:
        result = await engine.prompt("one bounded turn")
        snapshot = engine.execution_snapshot()
    finally:
        await engine.aclose()

    assert result.answer == "[tree token budget exhausted]"
    assert engine.stop_reason == "total_token_budget"
    assert len(client.calls) == 1
    assert snapshot["supervisor"]["total_tokens"] == 2
